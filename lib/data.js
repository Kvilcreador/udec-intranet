...
export async function addStudent(studentData) {
  ...
  try {
    ...
    // RACE CONDITION: Fetch vs 8s Timeout
    // This ensures we never hang forever, even if the server is slow
    const fetchPromise = fetch('/api/save-student', {
      method: 'POST',
    ...
    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(() => reject(new Error("API Proxy Timeout (8s)")), 8000)
    );
